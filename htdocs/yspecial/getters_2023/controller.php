...
if( !$self->get('release_date') ) {
	$now_date = date('Ymd') >= 20221209 ?date('Ymd') :'20221209';
...
if( $self->get('isp') == 10 ) {
	$up = ( $device_mode == "sp" ?'/sp' :'' ) .'/rakuten/event/getters_2023';
	if( $real_flag ) $up = 'https://lovefortune.zappallas.com/rakuten/event/getters_2023';
	$SmartyObj->assign( 'uri_path',			$up );
	$SmartyObj->assign( 'img_path',			$up );
	$SmartyObj->assign( 'special_title',	'ゲッターズ飯田の五星三心占い◆2023年あなたの運勢特集【楽天占い】' );
	$SmartyObj->assign( 'meta_keywords',	'ゲッターズ飯田,五星三心占い,2023年,運勢,無料,突然ですが占ってもいいですか' );
	$SmartyObj->assign( 'meta_description',	'芸能界最強の占い師・ゲッターズ飯田が、あなたの2023年を最高なものに変える！　6万人以上のデータから編み出した「五星三心占い」で、総合運・仕事運・恋愛運・健康運など訪れる運勢を丸ごと占います！' );
	$SmartyObj->assign( 'sns_description',	'芸能界最強の占い師・ゲッターズ飯田が、あなたの2023年を最高なものに変える！　6万人以上のデータから編み出した「五星三心占い」で、総合運・仕事運・恋愛運・健康運など訪れる運勢を丸ごと占います！' );
	$SmartyObj->assign( 'special_period',	'2023年12月9日(金)～2023年12月7日(木)' );

} else if( $self->get('isp') == 11 ) {
	$up = ( $device_mode == "sp" ?'/sp' :'' ) .'/cp_zap/getters_2023';
	//if( $real_flag ) $up = 'http://web-dev.ura.pga.jp/cp_zap/getters_2023';
	if( $real_flag ) $up = 'https://honkaku-uranai.jp/cp_zap/getters_2023';
	$SmartyObj->assign( 'uri_path',			$up );
	$SmartyObj->assign( 'img_path',			$up );
	$SmartyObj->assign( 'special_title',	'ゲッターズ飯田の五星三心占い◆2023年あなたの運勢特集' );
	$SmartyObj->assign( 'meta_keywords',	'ゲッターズ飯田,五星三心占い,2023年,運勢,無料,突然ですが占ってもいいですか' );
	$SmartyObj->assign( 'meta_description',	'芸能界最強の占い師・ゲッターズ飯田が、あなたの2023年を最高なものに変える！　6万人以上のデータから編み出した「五星三心占い」で、総合運・仕事運・恋愛運・健康運など訪れる運勢を丸ごと占います！' );
	$SmartyObj->assign( 'sns_description',	'芸能界最強の占い師・ゲッターズ飯田が、あなたの2023年を最高なものに変える！　6万人以上のデータから編み出した「五星三心占い」で、総合運・仕事運・恋愛運・健康運など訪れる運勢を丸ごと占います！' );
	$SmartyObj->assign( 'special_period',	'2023年12月9日(金)～2023年12月7日(木)' );
...
if( $cdn_flag && ( $self->get('isp') == 10 || $self->get('isp') == 11 ) ) {
	$img_path = 'https://lw6nr4oyej.user-space.cdn.idcfcloud.net/event/getters_2023';
	...
	if( ! $pack_flag && !$free_flag && ( $allfree_flag || $result_flag ) ) {
		...
		if( isset( $next ) ) {
			...
			foreach( $next as $next_id ) {
				...
				$data = array( 'menu_id' => $self->get('menu_id') ); //★開発用
				$min_title = array();
				...
				if( $self->get('menu_id') == 'gr23_001' && $data['menu_id'] == 'gr23_004' ) {
