...
{if $result_flag && $menu_category_name == 'aisho' }
    <meta property="og:image" content="https://honkaku-uranai.jp/cp_v/{$event}-{$menu_category_name}_v/img/oshi-{$menu_category_name}-sns.jpg" />
{else}
...
{if $isp == '11'}
<base href="http://web-dev06.ura.pga.jp/cp_zap/cp_v/{$event}/" />
{/if}
<!--- /SMARTY TEMPLATE テンプレート確認用のタグです削除してください --->
{/if}
{if !$result_flag}
...
{if $question_flag}
    <script type="text/javascript" src="{$img_path}/js/question.js"></script>
{/if}
{else}
...
{if $question_flag}
    <script type="text/javascript" src="{$uri_path}/js/question.js"></script>
{/if}
{/if}
...
{/if}
{if $question_flag}
    <link rel="stylesheet" type="text/css" href="{$img_path}/css/question.css">
{/if}
