
<!-- プロフィール -->
<div id="prof_group">
  <div class="section_bdy">
    <h2><img width="100%" height="auto" src="{$img_path}/img_sp/special/prof_group/section_title.jpg" alt="“芸能界最強”の占い師　ゲッターズ飯田"></h2>
    <div class="section_ftr">
      <div class="section_hdr">
        <div class="photo">
          <div class="detail_txt">
            <img width="100%" height="auto" src="{$img_path}/img_sp/special/prof_group/photo.png" alt="">
            <p>
これまで6万人を超える人を無償で占い続け{if $isp == '10'}<small>(※)</small>{/if}、「人の紹介がないと占わない」というスタンスが業界で話題に。20年以上占ってきた実績{if $isp == '10'}<small>(※)</small>{/if}を元に「五星三心占い」を編み出し、芸能界最強の占い師としてテレビ、ラジオに出演する他、雑誌やwebなどにも数多く登場する。<br><br>
メディアに出演するときは、自分の占いで「顔を出さないほうがいい」から赤いマスクを着けている。著書『ゲッターズ飯田の五星三心占い』は、シリーズ累計800万部を突破{if $isp == '10'}<small>(※)</small>{else}（2023年9月現在）{/if}。<br><br>
『ゲッターズ飯田の金持ち風水』『ゲッターズ飯田の運の鍛え方』『ゲッターズ飯田の裏運気の超え方』『ゲッターズ飯田の「五星三心占い」決定版』（以上、朝日新聞出版）、『ゲッターズ飯田の運命の変え方』（ポプラ社）、『開運レッスン』（セブン＆アイ出版）もそれぞれ10万部を発行{if $isp == '10'}<small>(※)</small>{/if}している。 
            </p>
          </div>
          <div class="clear"></div>
        </div>
        <div class="book_group">
          <div class="book_box">
          <div class="book_box_img">
          <img width="100%" height="auto" src="{$img_path}/img_sp/special/prof_group/img1.jpg" alt="">
          </div>
            <p>累計800万部{if $isp == '10'}<small>(※)</small>{/if}の本シリーズ「五星三心占い」。365日、まいにち開運！あなたの2023年を最高なものに変える「開運の書」表紙は、全12タイプそれぞれの2023年のラッキーカラー。<br>買ったその日から使えるお得な1冊です！{if $isp == '10'}<small>（※全て2022年9月時点）</small>{/if}</p>
          </div>

          <div class="link_box">
            <h4>【購入はこちら】</h4>
            <a href="https://books.rakuten.co.jp/rb/17195282/" target="_blank">金の羅針盤座</a>　
            <a href="https://books.rakuten.co.jp/rb/17195283/" target="_blank">銀の羅針盤座</a><br>
            <a href="https://books.rakuten.co.jp/rb/17195284/" target="_blank">金のインディアン座</a>　
            <a href="https://books.rakuten.co.jp/rb/17195285/" target="_blank">銀のインディアン座</a><br>
            <a href="https://books.rakuten.co.jp/rb/17195286/" target="_blank">金の鳳凰座</a>　
            <a href="https://books.rakuten.co.jp/rb/17195287/" target="_blank">銀の鳳凰座</a><br>
            <a href="https://books.rakuten.co.jp/rb/17195288/" target="_blank">金の時計座</a>　
            <a href="https://books.rakuten.co.jp/rb/17195289/" target="_blank">銀の時計座</a><br>
            <a href="https://books.rakuten.co.jp/rb/17195290/" target="_blank">金のカメレオン座</a>　
            <a href="https://books.rakuten.co.jp/rb/17195291/" target="_blank">銀のカメレオン座</a><br>
            <a href="https://books.rakuten.co.jp/rb/17195292/" target="_blank">金のイルカ座</a>　
            <a href="https://books.rakuten.co.jp/rb/17195293/" target="_blank">銀のイルカ座</a>
          </div>

        </div>
      </div>
    </div>
  </div>
</div>

<!-- //プロフィール -->

