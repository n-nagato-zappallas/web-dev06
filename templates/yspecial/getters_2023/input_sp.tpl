...
<!-- beginning -->
  <div id="header">
    <h2>{if $isp == '40'}<a href="http://www.nifty.com/{$event}/index.html">{else}<a href="../">{/if}<img width="100%" height="auto" src="/sp/cp_zap/getters_2023/img_sp/special/header_index.jpg" alt="{$special_title}"></a></h2>
  </div>
...
{if $menu_design_cd >= '15'}
                      <h3 class="menu_ttl_bar"><img width="100%" height="auto" src="/sp/cp_zap/event/getters_2023/img_sp/special/title/gr23_003.jpg" alt="“芸能界最強”の占い師ゲッターズ飯田があなたの2023年を最高なものに変える！　ゲッターズ飯田◇豪華DXパック　2023年あなたの3大運命　恋/仕事/総合　開運3か条・月運グラフ＆12年周期グラフ付き！"></h3>
{else}
...
{/if}
                                ...
                                <div class="clear"></div>
                                <p class="notice book_warning_2"><b>【ご購入の際の注意点】</b>本サービスは書籍『ゲッターズ飯田の五星三心占い2023』（朝日新聞出版）と内容・表現が重複している箇所がございます。また、書籍と同様に、様々なシチュエーションに応じたアドバイスが掲載されています。ご自身のライフステージに合わない部分も含めて、「開運の手引き」としてお楽しみいただくコンテンツです。ご購入においてはご了承の上ご購入手続きにお進みください。</p>
...
{if $isp == '11'}
              <div id="topicpath">
                <span class="def"></span><input type="hidden" name="contents_name" value="getters_2023" /><input type="hidden" name="menu_id" value="{$menu_id}" />
              </div>
{/if}
</article>
