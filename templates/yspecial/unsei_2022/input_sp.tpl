...
{if $isp == '11' && $menu_id == 'nw22_061' }
                                 <p style="line-height:1.5em;text-align:center;"><strong style="color:#A90C0C;">リニューアル記念★期間限定<br>無料会員の方は0円でご利用いただけます。</strong><br><strong>期間：2022年8月24日まで<br>{$menu_price} → </strong><strong style="color:#A90C0C;">0円</strong></p>
{else}
...
{if $isp == '11'}
              <div id="topicpath">
                <span class="def"></span><input type="hidden" name="contents_name" value="unsei_2022" /><input type="hidden" name="menu_id" value="{$menu_id}" />
              </div>
{/if}
</article>
